...
const dominio = (u) => { const m = norm(u).match(/^(?:[a-z]+:\/\/)?([^\/\s:?#]+)/); return m ? m[1].replace(/^www\./, "") : norm(u); };
function cer(a, b) { // tasa de error de caracteres, Levenshtein normalizada
  a = norm(a); b = norm(b);
...
(async () => {
  ...
  const ids = fs.readdirSync(dir).filter((f) => f.endsWith(".png")).map((f) => f.replace(/\.png$/, "")).filter((id) => verdad[id]).sort().slice(0, max);
  ...
  for (const id of ids) {
    ...
    porClase[esp] = porClase[esp] || { tot: 0, ok: 0 }; porClase[esp].tot++; if (obt === esp) porClase[esp].ok++;
    if (r.ok) {
      legibles++;
      const c = r.captura;
      if (norm(c.canal) === norm(v.canal)) campos.canal++;
      if (norm(c.remitente).includes(norm(v.remitente).slice(0, 8)) || norm(v.remitente).includes(norm(c.remitente).slice(0, 8))) campos.remitente++;
      const dv = new Set((v.enlaces || []).map(dominio)), dc = new Set((c.enlaces || []).map(dominio));
      if ([...dv].every((d) => dc.has(d)) && dc.size === dv.size) campos.enlaces++;
      const tv = new Set((v.telefonos || []).map((t) => String(t).replace(/\D/g, ""))), tc = new Set((c.telefonos || []).map((t) => String(t).replace(/\D/g, "")));
    ...
    filas.push({ id, esperado: esp, obtenido: obt, ok: obt === esp, ttft_vision: r.tiempos && r.tiempos.extraccion_ttft_ms, ms_vision: r.tiempos && r.tiempos.extraccion_ms, ttft_texto: r.tiempos && r.tiempos.veredicto_ttft_ms, ms_texto: r.tiempos && r.tiempos.veredicto_ms });
    process.stdout.write(`${filas.length}/${ids.length} ${id} -> ${obt}${obt === esp ? "" : "  (esperado " + esp + ")"}\n`);
  }
  ...
  const med = (k) => { const a = filas.map((f) => f[k]).filter((x) => typeof x === "number").sort((x, y) => x - y); return a.length ? a[Math.floor(a.length / 2)] : null; };
