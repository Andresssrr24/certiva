...
function ejecutar(cmd, args) {
  return new Promise((resolve) =>
    execFile(cmd, args, { timeout: 4000 }, (err, out) => {
      if (!err) return resolve(String(out));
      // lsof y pgrep terminan con código 1 cuando nada coincide: eso es «cero», no un fallo de medición.
      if (err.code === 1 && !err.killed) return resolve(String(out || ""));
      resolve(null);
    }),
  );
}
...
async function conexiones(pid, { pares = [] } = {}) {
  ...
  if (out === null)
    return {
      ...
      pares: null,
      detalle: [],
      error: "No se pudo medir TCP; no equivale a cero tráfico.",
    };
  const detalle = [];
