...
const ESQUEMA_VEREDICTO = {
  type: "object",
  additionalProperties: false,
  required: ["veredicto", "confianza", "senales", "accion", "canal_oficial"],
  properties: {
...
const PROMPT_TRANSCRIBIR = "Transcribe exactly all the text visible in this phone screenshot, in the original language, one line per visual line. Output only the transcription.";

// Prompt para el modelo de texto. Recibe la extracción y las evidencias de las reglas.
function promptVeredicto({ captura, senales, banco, base }) {
  const sistema = [
    ...
    "Presión de tiempo sin otra señal es sospechoso, no fraude.",
    ...
    "Parte del veredicto base que te dan las reglas y cámbialo solo con una razón concreta. No inventes tipos de señal: usa los de las reglas y, como máximo, una señal nueva con su frase exacta como evidencia.",
  ...
  const usuario = [
    ...
    "",
    "Devuelve el veredicto en el JSON pedido. Incluye en senales las evidencias de las reglas y las que tú detectes, cada una con su evidencia textual.",
  ].join("\n");
  return [
...
module.exports = { ESQUEMA_CAPTURA, ESQUEMA_VEREDICTO, PROMPT_CAPTURA, PROMPT_TRANSCRIBIR, promptVeredicto };
