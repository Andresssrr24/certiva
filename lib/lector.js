...
function remitenteDe(ls, canal) {
  ...
  for (const l of ls) { const m = l.match(/\b[A-Z][A-Z0-9-]{4,}\b/); if (m && !/^(SMS|HTTPS?|WHATSAPP)$/.test(m[0])) return m[0]; }
  return "";
}
...
function repararEnlaces(t) {
  return t
    ...
    .replace(/(https?:\/\/[a-z0-9-]+(?:\.[a-z0-9-]+)*)\s+(com|net|org|app|info|co|pa|link|ly|gd|gy|io|es|me)\b/gi, "$1.$2")
    ...
    .replace(/(https?:\/\/[a-z0-9-]+)\s+([a-z0-9-]+\.(?:com|net|org|app|info|co|pa|link|ly|gd|gy|io|es|me)\b(?:\/\S*)?)/gi, "$1$2")
...
function derivar(ls) {
  ...
  const enlaces = [...new Set((texto.match(RE_URL) || []).map((u) => u.replace(/[.,;:)]+$/, "")))].filter((u) => !/@/.test(u) && !/^\d/.test(u));
  ...
  const cuerpoLs = ls.filter((l) => !/^\d{1,2}[:.]\d{2}\b|\d{2}%$|^mensaje de texto|^no est[aá] en tus contactos|^este n[uú]mero no|^cuenta de empresa|^para m[ií]|^mensaje$|^[‹<>›|+]$|^[A-Z]$/i.test(l.trim()));
