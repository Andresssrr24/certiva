...
function bytesEnCache() {
  const m = new Map();
  try { for (const f of fs.readdirSync(CACHE_DIR)) { try { m.set(f.replace(/^[0-9a-f]{16}_/, ""), fs.statSync(path.join(CACHE_DIR, f)).size); } catch { /* */ } } } catch { /* sin caché */ }
  return m;
}
...
function idsEnCache() { return new Set([...bytesEnCache().keys()]); }
...
async function catalogo() {
  ...
  for (const [grupo, lista] of Object.entries(CATALOGO)) {
    ...
    for (const e of lista) {
      ...
      salida[grupo].push({ ...e, disponible: true, enCache, parcial, bytes, modelId: c.modelId, archivos: [c.modelId, p && p.modelId].filter(Boolean) });
    }
  }
  return salida;
}
...
async function argsCarga(e, { ctxSize = 4096 } = {}) {
  ...
  if (!p) return { modelSrc: c.src || c, modelType, modelConfig: (modelType === "llm" || /llamacpp/.test(String(c.engine || ""))) ? { ...cfg, ctx_size: ctxSize } : cfg };
...
module.exports = { CATALOGO, DEFECTOS, CACHE_DIR, sdk, entrada, catalogo, argsCarga, descargar, idsEnCache, bytesEnCache };
