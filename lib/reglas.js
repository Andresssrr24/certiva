...
function fraseDifusa(f) {
  // cada palabra de 5 letras o más admite un carácter cualquiera en vez de uno de los suyos
  const palabras = f
    .split(" ")
    .map((w) =>
      w.length >= 5
        ? `(?:${w}|${[...w].map((_, i) => w.slice(0, i) + "." + w.slice(i + 1)).join("|")})`
        : w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
    );
...
function evaluar(captura, banco) {
  ...
  if (pide || captura.pide_datos_sensibles) {
    senales.push({
      ...
      evidencia: pide
        ? `Pide datos que el banco nunca pide: «${pide[0].trim()}»`
