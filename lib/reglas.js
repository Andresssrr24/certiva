...
const RE_ENVIO_PARA_RECIBIR =
  /(envie|envia|transfiera|transfiere|deposite|mande|pague)[\s\S]{0,70}(para recibir|y reciba|y recibira|para duplicar|y le devolvemos|le devolvemos|para liberar|para desbloquear|para activar el retiro)|airdrop[\s\S]{0,80}(envie|transfiera|deposite)/i;
const RE_CAMBIO_DIRECCION =
  /(nueva|actualice|actualizo|cambio|cambie|use la nueva)[\s\S]{0,40}direccion de (deposito|billetera|wallet|pago|cobro|envio)|direccion de (deposito|billetera|wallet|pago|cobro|envio)[\s\S]{0,40}(cambio|nueva|actualiz)/i;
...
const RE_PIDE =
  /\b(env[ií]e|env[ií]a|responda|responde|confirme|confirma|indique|indica|comparta|comparte|ingrese|ingresa|digite|digita|proporcione|proporciona|necesito|d[ií]game|dime|escriba|escribe)\b[^.]{0,80}?\b(clave|contrase[ñn]a|pin|c[oó]digo|token|cvv|tarjeta|usuario|frase semilla|semilla|12 palabras|24 palabras|clave privada|llave privada)\b/i;
...
const RE_DICE_BANCO =
  /\b(banco|bancodemo|banca en l[ií]nea|su cuenta|tarjeta|pr[eé]stamo|cuenta)\b|billetera|wallet|exchange|usdt|cripto|criptomoneda|blockchain/i;
const RE_TERCERO =
...
function evaluar(captura, banco) {
  ...
  for (const enlace of captura.enlaces || []) {
    ...
    if (!dom) continue;
    // Una etiqueta suelta («app», «https://app») es una lectura partida, no un dominio: no se juzga.
    ...
    if (parecido)
      ...
      senales.push({
        ...
        evidencia: `Dice ser un canal oficial pero enlaza a ${dom}, que no lo es`,
      });
  }
  ...
  // Estafas de billetera: «envíe para recibir» y cambio de dirección de depósito. Mismas tácticas, mismo motor.

  const envio = RE_ENVIO_PARA_RECIBIR.exec(normaliza(texto));

  if (envio)
    senales.push({
      tipo: "envio_para_recibir",
      evidencia: `Pide enviar dinero para recibir más: «${envio[0].trim()}»`,
    });

  const direccion = RE_CAMBIO_DIRECCION.exec(normaliza(texto));

  if (direccion)
    senales.push({
      tipo: "cambio_direccion",
      evidencia: `Pide usar una dirección de pago nueva: «${direccion[0].trim()}»`,
    });
