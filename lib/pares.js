...
class Pares {
  ...
  estado() {
    return { nodo: this.nodo, pares: this.conexiones.size, indicadores: this.conocidos.size };
  }
  ...
  registrar(ind, origen) {
    ...
    if (!e) {
      e = { tipo: ind.tipo, hash: ind.hash, primero_ts: ind.ts || Date.now(), nodos: new Set(), cuenta: 0 };
      this.conocidos.set(clave, e);
    }
    if (e.nodos.has(origen)) return false;
    e.nodos.add(origen);
    e.cuenta = e.nodos.size;
    return true;
  }

  vecinos(ind) {
    const e = this.conocidos.get(`${ind.tipo}:${ind.hash}`);
    return e ? [...e.nodos].filter((n) => n !== this.nodo).length : 0;
  }
  ...
  publicar(ind) {
    const msg = {
      ...
      tipo: "indicador",
      nodo: this.nodo,
      ind: { tipo: ind.tipo, hash: ind.hash, ts: ind.ts || Date.now() },
    };
  ...
  _atender(conn, etiqueta) {
    ...
    const lote = [...this.conocidos.values()].map((e) => ({
      tipo: e.tipo,
      hash: e.hash,
      ts: e.primero_ts,
      nodos: [...e.nodos],
    ...
    const cerrar = () => {
      this.conexiones.delete(conn);
      this.onEstado(this.estado());
      this.onLog(`par desconectado (${etiqueta})`);
    };
  ...
  _mensaje(m) {
    ...
    if (m.tipo === "indicador" && m.ind && m.ind.hash) {
      ...
      for (const ind of m.inds.slice(0, 5000)) {
        ...
        let nuevo = false;
        for (const n of Array.isArray(ind.nodos) && ind.nodos.length ? ind.nodos : [m.nodo])
          if (n !== this.nodo && this.registrar(ind, n)) nuevo = true;
        if (nuevo)
          this.onIndicador({ tipo: ind.tipo, hash: ind.hash, ts: ind.ts, nodo: m.nodo, vecinos: this.vecinos(ind) });
      }
    }
  }
  ...
  async iniciarSwarm() {
    ...
    this.swarm.on("connection", (conn, info) =>
      this._atender(conn, `swarm ${info && info.publicKey ? info.publicKey.toString("hex").slice(0, 8) : ""}`),
    );
    this.swarm.join(TEMA, { server: true, client: true });
    this.swarm
      .flush()
      .then(() => this.onLog("swarm: anunciado en el DHT"))
      .catch((e) => this.onLog(`swarm: ${e.message}`));
  }
