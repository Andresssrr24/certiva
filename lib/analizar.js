...
class Motor {
  ...
  async analizar(rutaImagen) {
    ...
    if (ver.veredicto) {
      ...
      const adicionales = extra && extra.tipo && extra.tipo !== "ninguna" && extra.evidencia && !tipos.has(extra.tipo) ? [extra] : [];
      ...
      if (!Number.isFinite(conf)) conf = 0.5; else if (conf > 1) conf = Math.min(1, conf / 100);
