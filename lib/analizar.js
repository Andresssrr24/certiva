...
const { entradaTexto } = require("./entrada-texto");
const QUIET_MS = 120000;

// La segunda lectura con OCR solo puede SUMAR señales de frase. Las de dominio o número dependen de caracteres
// exactos que el OCR ensucia («https:Il», «bancodemo com pa»), y convertían correos oficiales en fraude.
// Los dominios los decide VisionPsy, y el contraste con OCR solo los corrige hacia el oficial, nunca al revés.
const SENALES_OCR_PERMITIDAS = new Set([
...
class Motor {
  ...
  async analizar(rutaImagen, options = {}) {
    return this._analizar({ rutaImagen }, options);
  }

  async analizarTexto(mensaje, options = {}) {
    return this._analizar({ mensaje }, options);
  }

  async _analizar({ rutaImagen, mensaje }, { onEtapa, onAlerta } = {}) {
    ...
    let alertaMs = null;
    aviso("vision", "inicio");
    const esTexto = mensaje !== undefined;
    const ext = esTexto ? entradaTexto(mensaje) : await this.extraerCaptura(rutaImagen);
    ...
    // Un dominio «parecido» puede ser el oficial mal transcrito: se contrasta con el OCR determinista. En correo siempre;
    // en SMS y WhatsApp solo si el dominio está a una o dos letras del oficial («bancodesmo», «banc.codemo»), que es la
    // firma de un error de lectura. Un dominio de phishing real («bancodemo-pa.app») queda lejos y no se contrasta.
    let contraste = null;
    const esDominio = (x) => x.tipo === "dominio_parecido" || x.tipo === "dominio_no_oficial";
    const casiOficial = (x) => {
      const mm = x.evidencia.match(/dominio ([a-z0-9.-]+)|enlaza a ([a-z0-9.-]+)/i);
      const plano = (d) =>
        String(d || "")
      ...
      return !!dom && this.banco.dominios_oficiales.some((o) => reglas.levenshtein(dom, plano(o)) <= 2);
    };
    if (
      !esTexto &&
      this.contrasteOcr &&
      senales.some(esDominio) &&
      (ext.captura.canal === "correo" || senales.some((x) => esDominio(x) && casiOficial(x)))
    ) {
      aviso("ocr", "inicio");
      const t2 = Date.now();
      try {
    ...
    let revision = null;
    if (!esTexto && base === "sin_senales" && this.lectorKey !== "ocr") {
      aviso("ocr", "inicio");
      const t = Date.now();
      try {
        const segunda = await this.extraerConOcr(rutaImagen);
        const otras = reglas.evaluar(segunda.captura, this.banco).filter((x) => SENALES_OCR_PERMITIDAS.has(x.tipo));
        ...
        if (otroBase !== "sin_senales") {
          senales = [...senales, ...otras.map((x) => ({ ...x, evidencia: `Segunda lectura OCR: ${x.evidencia}` }))];
          base = otroBase;
          revision.estado = "alerta_segunda_lectura";
        } else if (a.size < 4 || acuerdo < 0.3) {
          // El OCR incluye la interfaz (hora, batería, «Mensaje de texto») y VisionPsy no: con 0,65 de Jaccard
          // siete mensajes cortos legítimos quedaban como «no legible». 0,3 separa lecturas de pantallas distintas.
    ...
    // Advertir antes de esperar la explicación generativa; nunca anticipar ausencia de señales.
    if (onAlerta && ["fraude", "sospechoso"].includes(base)) {
      const { asegurarVeredicto } = require("./seguridad");
      alertaMs = Date.now() - inicio;
      const alerta = {
        ok: true,
        preliminar: true,
        origen: esTexto ? "texto" : "captura",
        captura: ext.captura,
        senales,
        veredicto_reglas: base,
        veredicto: asegurarVeredicto(null, base, senales, this.banco),
        tiempos: { alerta_ms: alertaMs },
      };
      try {
        onAlerta(alerta);
      } catch {
        /* La vista no interrumpe el análisis. */
      }
    }
