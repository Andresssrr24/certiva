...
const evidence = {
  ...
  integration: "HTTP bridge + real @qvac/sdk 0.19.0",
  hardware: { cpu: cpus()[0]?.model, ramGiB: Math.round(totalmem() / 1073741824), platform: platform(), arch: arch() },
  cases: [],
};
try {
  ...
  const response = await fetch(base + "/analyze-image", {
    method: "POST",
    headers,
  ...
  evidence.cases.push({
    name: "capture-phishing",
    input: "data/capturas/fraude-bloqueo_enlace-01.png (synthetic Banco Demo)",
    ...result,
  });
