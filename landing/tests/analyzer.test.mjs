...
import { analyzeText, EXAMPLES, extractDomains } from "../public/analyzer.js";
...
test("does not authenticate lookalikes, deceptive suffixes or URL userinfo", () => {
  for (const url of [
    "https://cajadeahorros.com.pa.evil.example",
    "https://cajadeahorros.com.pa@evil.example",
    "https://cajadeahorr0s.com.pa",
    "https://bit.ly/abc",
    "https://192.168.1.1/login",
  ])
    assert.equal(analyzeText("Caja de Ahorros: revisa tu cuenta en " + url).verdict, "fraude", url);
...
test("official domain and missing input are handled without a safe verdict", () => {
  assert.equal(
    analyzeText("Visita directamente https://www.cajadeahorros.com.pa para consultar información.").verdict,
    "sin_senales",
  );
...
test("urgency alone is suspicious, not confirmed fraud", () =>
  assert.equal(analyzeText("Último aviso: la solicitud vence hoy mismo.").verdict, "sospechoso"));
