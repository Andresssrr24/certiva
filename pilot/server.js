...
const allowedFiles = {
  ...
  "/certiva.js": ["certiva.js", "text/javascript"],
  "/policy-key.js": ["policy-key.js", "text/javascript"],
  "/policy.json": ["policy.json", "application/json"],
  "/brand.png": ["brand.png", "image/png"],
  "/manrope-800.ttf": ["manrope-800.ttf", "font/ttf"],
};
...
function createApp({ file = ":memory:", accounts = [] } = {}) {
  ...
  function audit(user, caseId, event, detail = {}) {
    db.prepare("INSERT INTO audit(tenant,actor,case_id,event,created,detail) VALUES(?,?,?,?,?,?)").run(
      user.tenant,
      user.id,
      caseId,
      event,
  ...
  function session(req) {
    ...
    const user = db
      .prepare(
        "SELECT u.id,u.tenant,u.role,s.csrf,s.id AS session_id FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.expires>?",
      )
      .get(hash(value), Date.now());
    if (!user) fail(401, "La sesión venció. Vuelve a ingresar");
    if (!["GET", "HEAD"].includes(req.method) && req.headers["x-csrf-token"] !== user.csrf)
  ...
  async function body(req) {
    ...
    try {
      ...
      if (!data || Array.isArray(data) || typeof data !== "object") fail(400, "JSON inválido");
      return data;
    } catch {
  ...
  function validateReport(data) {
    const supportedEngine =
      (SDK.SOURCES.includes(data.source) && data.sdkVersion === SDK.VERSION) ||
    ...
    if (
      !SDK.isUUID(data.assessmentId) ||
      data.consent !== true ||
      !Object.hasOwn(SDK.LABELS, data.outcome) ||
      !SDK.CHANNELS.includes(data.channel) ||
      !supportedEngine ||
      data.policyVersion !== POLICY_VERSION ||
      !Array.isArray(data.reasonCodes) ||
      data.reasonCodes.length > Object.keys(SDK.REASONS).length ||
      data.reasonCodes.some((code) => !Object.hasOwn(SDK.REASONS, code)) ||
      ...
      Date.parse(data.evaluatedAt) > Date.now() + 300000 ||
      Date.parse(data.evaluatedAt) < Date.now() - 86400000
    )
      fail(400, "Reporte inválido o vencido. Vuelve a verificar el mensaje");
    const codes = data.reasonCodes;
    const expected = codes.some((c) => ["politica_vencida", "lectura_incompleta"].includes(c))
      ? "no_concluyente"
      : codes.some((c) =>
            [
    ...
    try {
      ...
      if (url.pathname === "/api/login" && req.method === "POST") {
        ...
        if (
          ...
          fail(400, "Credenciales inválidas");
        ...
        if (!user || !crypto.timingSafeEqual(Buffer.from(check, "hex"), Buffer.from(user.password, "hex"))) {
          attempts.set(peer, {
            until: count?.until > now ? count.until : now + 300000,
            total: count?.until > now ? count.total + 1 : 1,
          });
        ...
        send(
          200,
          { id: user.id, role: user.role, csrf },
          { "Set-Cookie": `certiva=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=3600` },
        );
        return;
      }
      ...
      if (url.pathname === "/api/session" && req.method === "GET") {
        send(200, { id: user.id, role: user.role, csrf: user.csrf });
        return;
      }
      if (url.pathname === "/api/logout" && req.method === "POST") {
        transaction(() => {
          db.prepare("DELETE FROM sessions WHERE id=?").run(user.session_id);
          audit(user, null, "sesion_cerrada");
        });
        send(200, { ok: true }, { "Set-Cookie": "certiva=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0" });
        return;
      }
      if (url.pathname === "/api/cases" && req.method === "POST") {
        ...
        const result = transaction(() => {
          const existing = db
            .prepare("SELECT * FROM cases WHERE tenant=? AND actor=? AND assessment=?")
            .get(user.tenant, user.id, data.assessmentId);
          const canonical = JSON.stringify(Object.fromEntries(REPORT_KEYS.map((key) => [key, data[key]])));
          if (existing) {
            if (existing.report !== canonical) fail(409, "El identificador ya corresponde a otro reporte");
            return { row: existing, duplicate: true };
          }
          const count = db
            .prepare("SELECT count(*) n FROM cases WHERE actor=? AND tenant=? AND created>?")
            .get(user.id, user.tenant, Date.now() - 3600000).n;
          if (count >= 30) fail(429, "Límite de reportes por hora alcanzado");
          const id = crypto.randomUUID();
          const now = Date.now();
          db.prepare("INSERT INTO cases(id,tenant,actor,assessment,report,created,updated) VALUES(?,?,?,?,?,?,?)").run(
            id,
            user.tenant,
            user.id,
            data.assessmentId,
        ...
        send(result.duplicate ? 200 : 201, project(result.row, user.role));
        return;
      }
      if (url.pathname === "/api/cases" && req.method === "GET") {
        const rows =
          user.role === "cliente"
            ? db
                .prepare("SELECT * FROM cases WHERE tenant=? AND actor=? ORDER BY created DESC LIMIT 200")
                .all(user.tenant, user.id)
            : db.prepare("SELECT * FROM cases WHERE tenant=? ORDER BY created DESC LIMIT 200").all(user.tenant);
        audit(user, null, "bandeja_consultada", { count: rows.length });
        send(200, { cases: rows.map((row) => project(row, user.role)), limit: 200 });
        return;
      }
      ...
      if (match && req.method === "PATCH") {
        ...
        const data = await body(req);
        if (
          Object.keys(data).some((key) => !["version", "action", "resolution"].includes(key)) ||
          !Number.isInteger(data.version)
        )
          fail(400, "Cambio inválido");
        const row = transaction(() => {
          ...
          let assignee;
          let resolution;
          if (data.action === "tomar" && old.state === "nuevo" && !data.resolution) {
            state = "en_revision";
            assignee = user.id;
            resolution = null;
          } else if (
            data.action === "resolver" &&
            old.state === "en_revision" &&
            old.assignee === user.id &&
            RESOLUTIONS.includes(data.resolution)
          ) {
            state = "resuelto";
            assignee = user.id;
          ...
          db.prepare(
            "UPDATE cases SET state=?,assignee=?,resolution=?,updated=?,version=version+1 WHERE id=? AND tenant=?",
          ).run(state, assignee, resolution, Date.now(), old.id, user.tenant);
          audit(user, old.id, data.action === "tomar" ? "caso_asignado" : "caso_resuelto", { resolution });
          return getCase(user, old.id);
        });
        send(200, project(row, user.role));
        return;
      }
      if (url.pathname === "/api/audit" && req.method === "GET") {
        ...
        send(200, {
          events: db
            .prepare(
              "SELECT seq,actor,case_id,event,created,detail FROM audit WHERE tenant=? ORDER BY seq DESC LIMIT 200",
            )
            .all(user.tenant),
        });
        return;
      ...
      if (!res.headersSent)
        send(error.status || 500, { error: error.status ? error.message : "No pudimos completar la operación" });
      else res.end();
    }
  });
...
if (require.main === module) {
  ...
  if (!fs.existsSync(bootstrap)) {
    const accounts = ["cliente", "analista", "auditor"].map((role) => ({
      id: role,
      tenant: "caja-piloto",
      role,
  ...
  const app = createApp({
    file: path.join(folder, "cases.sqlite"),
    accounts: JSON.parse(fs.readFileSync(bootstrap, "utf8")),
  });
