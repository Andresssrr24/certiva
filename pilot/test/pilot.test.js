...
function assess(value = text, extra = {}) { return SDK.assess({ id: crypto.randomUUID(), text: value, channel: "whatsapp", source: "texto", now: fixedNow, ...extra }, policy); }
test("texto sospechoso y negaciones legítimas no reciben el mismo resultado", () => {
  assert.equal(assess().outcome, "riesgo");
  assert.equal(assess("Tu estado de cuenta está disponible. Nunca compartas claves ni códigos.").outcome, "sin_senales");
...
test("lectura vacía, OCR sin confirmar y política vencida se abstienen", () => {
  for (const result of [assess("hola"), assess(text, { source: "apple_vision" }), assess(text, { now: Date.parse("2027-01-01") })]) assert.equal(result.outcome, "no_concluyente");
});
test("un enlace reconocido no autentica al remitente", () => {
  ...
  assert.equal(result.outcome, "sin_senales"); assert.match(result.action, /no confirma/);
});
...
test("el reporte excluye texto, enlaces y números aun si se agregan al resultado", () => {
  const result = assess(); Object.assign(result, { text, url: "https://malicioso.example", phone: "6000-0000" });
  const report = SDK.makeReport(result);
  assert.deepEqual(Object.keys(report).sort(), ["assessmentId", "outcome", "reasonCodes", "channel", "source", "evaluatedAt", "policyVersion", "sdkVersion", "consent"].sort());
...
test("el paquete ejecuta sin require, red o APIs de Node y conserva resultados", () => {
  const context = vm.createContext({}); vm.runInContext(fs.readFileSync(path.join(__dirname, "../public/certiva.js"), "utf8"), context);
...
async function setup(t, options = {}) {
  ...
  async function login(username) {
    const response = await fetch(base + "/api/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, password: "test-password" }) });
  ...
  async function request(who, url, method = "GET", data, headers = {}) {
    const response = await fetch(base + url, { method, headers: { "Content-Type": "application/json", Cookie: who?.cookie || "", "X-CSRF-Token": who?.csrf || "", ...headers }, ...(data ? { body: JSON.stringify(data) } : {}) });
    return { status: response.status, body: await response.json() };
  }
  return { ...app, base, login, request };
}
function report() { return { ...SDK.makeReport(assess()), evaluatedAt: new Date().toISOString() }; }
test("reporte → asignación → resolución, con auditoría y sin texto persistido", async t => {
  const a = await setup(t); const client = await a.login("client-a"); const analyst = await a.login("analyst-a");
  const created = await a.request(client, "/api/cases", "POST", report()); assert.equal(created.status, 201);
  const id = created.body.id;
  assert.equal((await a.request(analyst, `/api/cases/${id}`, "PATCH", { version: 1, action: "resolver", resolution: "fraude_confirmado" })).status, 409);
  const taken = await a.request(analyst, `/api/cases/${id}`, "PATCH", { version: 1, action: "tomar" }); assert.equal(taken.body.state, "en_revision");
  const closed = await a.request(analyst, `/api/cases/${id}`, "PATCH", { version: 2, action: "resolver", resolution: "sin_evidencia" }); assert.equal(closed.body.state, "resuelto");
  const audit = await a.request(analyst, "/api/audit"); assert.ok(audit.body.events.some(e => e.event === "caso_resuelto"));
...
test("reportes idempotentes, conflictos de contenido y concurrencia", async t => {
  const a = await setup(t); const client = await a.login("client-a"); const analyst = await a.login("analyst-a"); const data = report();
  const first = await a.request(client, "/api/cases", "POST", data);
  const repeat = await a.request(client, "/api/cases", "POST", data); assert.equal(repeat.status, 200); assert.equal(repeat.body.id, first.body.id);
  ...
  assert.equal((await a.request(analyst, `/api/cases/${first.body.id}`, "PATCH", { version: 1, action: "tomar" })).status, 409);
});
test("aislamiento de bancos, de clientes y permisos de auditor", async t => {
  ...
  for (const id of ["client-b", "other-client-a", "analyst-b"]) {
    const user = await a.login(id); assert.equal((await a.request(user, "/api/cases")).body.cases.length, 0);
  }
  const other = await a.login("analyst-b"); assert.equal((await a.request(other, `/api/cases/${created.body.id}`, "PATCH", { version: 1, action: "tomar" })).status, 404);
  const auditor = await a.login("auditor-a"); assert.equal((await a.request(auditor, `/api/cases/${created.body.id}`, "PATCH", { version: 1, action: "tomar" })).status, 403);
...
test("rechaza contenido privado adicional, consentimiento ausente, versiones y resultados inválidos", async t => {
  const a = await setup(t); const client = await a.login("client-a");
  for (const extra of [{ text }, { tenant: "bank-b" }, { consent: false }, { sdkVersion: "fake" }, { outcome: "sin_senales" }, { reasonCodes: ["<script>"] }, { evaluatedAt: "ayer" }]) {
...
test("sin sesión, CSRF, origen cruzado, host ajeno y sesión cerrada", async t => {
  const a = await setup(t); assert.equal((await a.request(null, "/api/cases")).status, 401);
  ...
  const hostStatus = await new Promise((resolve, reject) => {
    http.get(a.base + "/api/cases", { headers: { Host: "evil.example", Cookie: client.cookie } }, response => { response.resume(); resolve(response.statusCode); }).on("error", reject);
  });
  assert.equal(hostStatus, 403);
  await a.request(client, "/api/logout", "POST"); assert.equal((await a.request(client, "/api/cases")).status, 401);
});
...
test("los casos persisten al volver a abrir la base", async t => {
  ...
  const file = path.join(folder, "cases.sqlite"); const first = createApp({ file, accounts });
  first.db.prepare("INSERT INTO cases(id,tenant,actor,assessment,report,created,updated) VALUES(?,?,?,?,?,?,?)").run(crypto.randomUUID(), "bank-a", "client-a", crypto.randomUUID(), "{}", Date.now(), Date.now());
  first.db.close(); const second = createApp({ file }); assert.equal(second.db.prepare("SELECT count(*) n FROM cases").get().n, 1); second.db.close();
});
...
test("señales de pago conservan explicación y paridad en web, iOS y Android", () => {
  ...
  for (const resource of ["public/certiva.js", "ios/Sources/CertivaSDK/Resources/certiva.js", "android-app/sdk/src/main/assets/certiva.js"]) {
...
test("la consola acepta las señales de pago emitidas por el motor vigente", async t => {
  const app = await setup(t); const client = await app.login("client-a");
  for (const message of ["Envíe 100 para recibir su beneficio.", "Use la nueva dirección de pago indicada por su proveedor."]) {
