...
if(!serial||!apk||!model||!/^local\.certiva\.[a-z]+$/.test(packageName)){console.error('node provision.cjs SERIAL APK MODEL.gguf [local.certiva.qvacprobe]');process.exit(1);}
...
(async()=>{
 const hash=crypto.createHash('sha256');for await(const chunk of fs.createReadStream(model))hash.update(chunk);
 if(hash.digest('hex')!==expected)throw Error('Model checksum mismatch');
 const state=execFileSync(adb,['-s',serial,'get-state'],{encoding:'utf8'}).trim();if(state!=='device')throw Error('Authorize USB debugging first');
 const remoteApk='/data/local/tmp/certiva-install.apk';
 run('push','-Z',apk,remoteApk);run('shell','pm','install','-r',remoteApk);run('shell','rm',remoteApk);
 const remote='/data/local/tmp/certiva-qwen3.gguf';run('push','-Z',model,remote);
 run('shell',`run-as ${packageName} mkdir -p files/qvac`);
 run('shell',`run-as ${packageName} cp ${remote} files/qvac/Qwen3-1.7B-Q4_K_M.gguf`);
 const output=execFileSync(adb,['-s',serial,'shell',`run-as ${packageName} sha256sum files/qvac/Qwen3-1.7B-Q4_K_M.gguf`],{encoding:'utf8'});
