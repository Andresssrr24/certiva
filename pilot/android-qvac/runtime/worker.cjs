...
let fatal = null;
Bare.on('uncaughtException', () => { fatal = 'El motor local dejó de responder'; });
Bare.on('unhandledRejection', () => { fatal = 'El motor local dejó de responder'; });
...
BareKit.on('push', async (payload, reply) => {
  ...
  try {
    ...
    if(request.op==='status') {reply(null,JSON.stringify({ready:!!model,runtime:'QVAC llm-llamacpp 0.49.1',platform:Bare.platform,arch:Bare.arch}));return;}
    if(request.op==='load') {
      if(model)await model.unload();
      model=null;
      const candidate=new Llm({files:{model:[request.modelPath]},config:{device:'cpu',gpu_layers:'0',ctx_size:'2048',predict:'160',temp:'0',threads:'4',backendsDir:request.nativeLibraryDir},opts:{stats:true},logger:{debug(){},info(){},warn(){},error(){}}});
      await candidate.load();model=candidate;
      reply(null,JSON.stringify({ready:true,runtime:'QVAC llm-llamacpp 0.49.1'}));return;
    }
    if(request.op==='unload'){if(model)await model.unload();model=null;reply(null,'{}');return;}
    ...
    const response=await model.run([
      ...
      {role:'assistant',content:'{"pide_datos_sensibles":true,"pago_terceros":false,"urgencia":false,"uncertain":false}'},
      {role:'user',content:'Consejo de seguridad: no reveles a nadie el PIN de tu tarjeta.'},
      {role:'assistant',content:'{"pide_datos_sensibles":false,"pago_terceros":false,"urgencia":false,"uncertain":false}'},
    ...
    try {output=await response.await();}finally{clearTimeout(timeout);}
    ...
    const result=JSON.parse(visible.trim().replace(/^```(?:json)?\s*/, '').replace(/\s*```$/, ''));
    if(!result || Array.isArray(result) || Object.keys(result).length!==fields.length || fields.some(key=>typeof result[key]!=='boolean'))throw Error('Respuesta del modelo no válida');
    const signals=fields.filter(key=>key!=='uncertain'&&result[key]);
    reply(null,JSON.stringify({signals,uncertain:result.uncertain,elapsedMs:Date.now()-started,backend:response.stats?.backendDevice||'cpu',model:'Qwen3 1.7B Q4_K_M',runtime:'QVAC llm-llamacpp 0.49.1'}));
  }catch(error){reply(null,JSON.stringify({error:String(error.message||error).slice(0,180)}));}
  finally{busy=false;}
});
