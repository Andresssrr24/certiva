...
public final class ProtectionDetailActivity extends Activity {
    ...
    private TextView text(String value,int size,boolean bold){return ProtectionStyle.text(content,value,size,bold);}
    ...
    private void render(){
        content=ProtectionStyle.screen(this);ProtectionStyle.header(content,"Detalle de alerta");ProtectionStyle.shield(content);
        text("PROTECCIÓN · WHATSAPP",11,true);
        ...
        if("unavailable".equals(result.optString("aiStatus")))text("IA no disponible. Esta alerta se basa en reglas de texto locales.",14,false);
        LinearLayout page=content;content=ProtectionStyle.card(page,0xffedf1f8);
        text("Señales encontradas",13,true);
        var reasons=result.optJSONArray("reasons");if(reasons!=null)for(int i=0;i<reasons.length();i++)text(reasons.optJSONObject(i).optString("title"),16,true);
        content=ProtectionStyle.card(page,ProtectionStyle.TONAL);text("Qué puedes hacer ahora",13,true);text(result.optString("action"),17,false);content=page;
        ...
        ProtectionStyle.disclosure(content,"Cómo se analizó",result.optString("coverage")+" · "+result.optString("policyVersion")+(result.has("aiModel")?"\n"+result.optString("aiModel")+" · Android · "+result.optLong("aiElapsedMs")+" ms":""));
        text("Detectado: "+java.text.DateFormat.getDateTimeInstance().format(new java.util.Date(result.optLong("receivedAt"))),12,false);
