...
const nombres = ["Carlos Pérez", "Ana Gómez", "Luis Batista", "María Rodríguez", "José Castillo", "Yaritza Moreno", "Roberto Herrera", "Katherine De León"];
const lugares = ["Albrook", "Multiplaza", "David", "Santiago", "Chitré", "Colón", "Penonomé", "Vía España", "Costa del Este", "La Chorrera"];
const comercios = ["Super 99", "Riba Smith", "Farmacias Arrocha", "Amazon", "Netflix", "El Machetazo", "Uber", "PriceSmart"];
const fechas = ["3 de septiembre", "5 de septiembre", "8 de septiembre", "9 de septiembre", "10 de septiembre", "12 de septiembre", "15 de septiembre"];
const horas = () => `${Math.floor(R() * 12) + 1}:${pick(["05", "12", "24", "37", "48", "53"])}`;
const codigo = () => digitos(6);
const parecido = () => pick(banco.dominios_parecidos);
const acortado = () => `https://${pick(banco.acortadores)}/${digitos(2)}${pick(["Kd", "xQ", "Bz", "pT", "Lm"])}${digitos(2)}`;
const ip = () => `http://${pick([181, 186, 190, 200])}.${digitos(2)}.${digitos(3)}.${digitos(2)}/bancodemo/${pick(["verificar", "acceso", "seguridad"])}`;
...
const FRAUDE = {
  bloqueo_enlace: () => {
    ...
    return { canal: pick(["sms", "whatsapp"]), remitente: pick([movilConPrefijo(), `+1 (${digitos(3)}) ${digitos(3)}-${digitos(4)}`, "ALERTA-BANCO"]), texto: t, enlaces: [`https://${d}/${t.split(`https://${d}/`)[1].split(/[\s.]/)[0]}`], telefonos: [], montos: [], pide: false, urgencia: true, senales: ["dominio_parecido", "urgencia"] };
  },
  reembolso_acortador: () => {
    const m = monto(); const e = acortado();
    const t = pick([
    ...
    return { canal: pick(["sms", "whatsapp"]), remitente: pick([movilConPrefijo(), "PROMO", movilConPrefijo()]), texto: t, enlaces: [e], telefonos: [], montos: [m], pide: false, urgencia: urg, senales: ["acortador", ...(urg ? ["urgencia"] : [])] };
  },
  pide_codigo: () => {
    const t = pick([
    ...
    return { canal: pick(["sms", "whatsapp", "sms"]), remitente: pick([movilConPrefijo(), "BANC0DEMO", movilConPrefijo()]), texto: t, enlaces: [], telefonos: [], montos: [], pide: true, urgencia: urg, senales: ["pide_datos_sensibles", ...(urg ? ["urgencia"] : [])] };
  },
  pago_yappy: () => {
    const m = monto(); const n = movil();
    const t = pick([
    ...
    return { canal: pick(["sms", "whatsapp"]), remitente: pick([movilConPrefijo(), "COBROS"]), texto: t, enlaces: [], telefonos: [n], montos: [m], pide: false, urgencia: true, senales: ["pago_terceros", "numero_no_oficial", "urgencia"] };
  },
  correo_actualizacion: () => {
    ...
    return { canal: "correo", remitente: pick([`seguridad@${d}`, `notificaciones@${d}`, `alertas@${d}`]), remitenteNombre: pick([`${banco.nombre} Seguridad`, banco.nombre, `Alertas ${banco.nombre}`]), asunto: pick(["Actualización obligatoria de datos", "Acción requerida: verifique su cuenta", "Inconsistencia en su estado de cuenta"]), texto: t, enlaces: [`https://${d}/${t.split(`https://${d}/`)[1].split(/[\s.]/)[0]}`], telefonos: [], montos: [], pide: true, urgencia: true, senales: ["dominio_parecido", "pide_datos_sensibles", "urgencia"] };
  },
  ejecutivo_whatsapp: () => {
    ...
    return { canal: "whatsapp", remitente: movilConPrefijo(), texto: t, enlaces: [], telefonos: [], montos: [], pide: true, urgencia: false, senales: ["pide_datos_sensibles"] };
  },
  compra_no_reconocida: () => {
    const d = parecido(); const m = monto(); const c = pick(comercios);
    const t = pick([
    ...
    return { canal: "sms", remitente: pick([movilConPrefijo(), "ALERTAS", `+1 (${digitos(3)}) ${digitos(3)}-${digitos(4)}`]), texto: t, enlaces: [`https://${d}/${t.split(`https://${d}/`)[1].split(/[\s.]/)[0]}`], telefonos: [], montos: [m], pide: false, urgencia: true, senales: ["dominio_parecido", "urgencia"] };
  },
  ip_literal: () => {
    const e = ip();
    const t = pick([
    ...
    return { canal: pick(["sms", "whatsapp"]), remitente: pick([movilConPrefijo(), "SEGURIDAD"]), texto: t, enlaces: [e], telefonos: [], montos: [], pide: false, urgencia: urg, senales: ["ip_literal", ...(urg ? ["urgencia"] : [])] };
  },
};
...
const LEGITIMO = {
  alerta_transaccion: () => {
    const m = monto(); const tel = telOficial();
    const t = pick([
    ...
    return { canal: "sms", remitente: senderOficialSms, texto: t, enlaces: [], telefonos: tels, montos: [m], pide: false, urgencia: false, senales: [], veredicto: "sin_senales" };
  },
  otp_legitimo: () => {
    const c = codigo();
    const t = pick([
    ...
    return { canal: "sms", remitente: senderOficialSms, texto: t, enlaces: [], telefonos: [], montos: [], pide: false, urgencia: false, senales: [], veredicto: "sin_senales" };
  },
  recordatorio_pago: () => {
    const m = monto();
    const t = pick([
    ...
    return { canal: "sms", remitente: senderOficialSms, texto: t, enlaces: [oficialApp], telefonos: [], montos: [m], pide: false, urgencia: false, senales: [], veredicto: "sin_senales" };
  },
  correo_estado_cuenta: () => {
    ...
    return { canal: "correo", remitente: "notificaciones@bancodemo.com.pa", remitenteNombre: banco.nombre, asunto: pick([`Su estado de cuenta de ${mes} está disponible`, "Información de horarios de atención"]), texto: t, enlaces: [oficialApp], telefonos: [], montos: [], pide: false, urgencia: false, senales: [], veredicto: "sin_senales" };
  },
  whatsapp_oficial: () => {
    const t = pick([
    ...
    return { canal: "whatsapp", remitente: "Banco Demo +507 300-1234", verificado: true, texto: t, enlaces: [], telefonos: ["+507 300-1234"], montos: [], pide: false, urgencia: false, senales: [], veredicto: "sin_senales" };
  },
  neutral: () => {
    const t = pick([
    ...
    return { canal: pick(["sms", "whatsapp"]), remitente: pick([movilConPrefijo(), pick(nombres), "ENVIOS"]), texto: t, enlaces: [], telefonos: [], montos: [], pide: false, urgencia: false, senales: [], veredicto: "sin_senales" };
  },
  oficial_con_urgencia: () => {
    ...
    return { canal: "sms", remitente: senderOficialSms, texto: t, enlaces: t.includes(oficialApp) ? [oficialApp] : [], telefonos: [], montos: [], pide: false, urgencia: true, senales: ["urgencia"], veredicto: "sospechoso" };
  },
};
...
function generar({ porTipo = 8 } = {}) {
  ...
  const agrega = (grupo, tipo, fn, veredictoBase) => {
    for (let i = 0; i < porTipo; i++) {
      ...
      mensajes.push({
        id, grupo, tipo, canal: m.canal, remitente: m.remitente, remitenteNombre: m.remitenteNombre, asunto: m.asunto, verificado: !!m.verificado, hora: horas(),
        texto: m.texto, enlaces: m.enlaces, telefonos: m.telefonos, montos: m.montos,
        pide_datos_sensibles: m.pide, urgencia: m.urgencia,
...
function escribir(mensajes) {
  ...
  const verdad = {};
  for (const m of mensajes) {
    verdad[m.id] = { canal: m.canal, remitente: m.remitente, texto: m.texto, enlaces: m.enlaces, telefonos: m.telefonos, montos: m.montos, pide_datos_sensibles: m.pide_datos_sensibles, urgencia: m.urgencia, esperado: m.esperado };
  }
  fs.writeFileSync(path.join(__dirname, "verdad.json"), JSON.stringify(verdad, null, 2));
  return { total: mensajes.length, fraude: mensajes.filter((m) => m.grupo === "fraude").length, legitimo: mensajes.filter((m) => m.grupo === "legitimo").length, dir };
}
