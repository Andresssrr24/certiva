...
Corte documental: **11 de septiembre de 2026, hora de Panamá**. Los [avances de todas las tareas](docs/AVANCES-HILOS-20260911.md) reúnen código, pruebas, documentación, videos e investigación, con sus PRs y artefactos. El inventario actualizado incluye las PR #37/#38 integradas en `main` y los 22 archivos de la prerelease, con sus hashes. API36 entró en `main` con la PR #37, que reemplaza la #36; su PR anterior se había fusionado en otra rama. Las pruebas y limitaciones se declaran por componente; los benchmarks históricos no equivalen a una evaluación de esta versión.
...
Registro beta: **habilitado con Google; descarga cerrada pendiente**. Confirma la cuenta y su membresía en el grupo con consentimiento. `BETA_ENABLED=true` permite registrarse; `BETA_PLAY_READY=false` mantiene oculto el enlace de instalación. Con la prueba cerrada disponible y la bandera activada, el registro confirmado redirigirá directamente a Google Play. La tarea de landing verificó OAuth completo en Chrome de escritorio con una cuenta ya perteneciente al grupo, tras corregir el fallo de origen del formulario. Bryan confirmó después que el registro en Android funciona. Siguen pendientes una nueva alta externa instrumentada y la distribución cerrada. [Configuración y estado](landing/BETA-SETUP.md) · [Revisión de integración](landing/BETA-REVIEW.md). La tarea propietaria confirmó el envío de 14 cambios a revisión para **0.5.0 · Certiva · Prueba cerrada**, con el único AAB versión 5. La ficha, Seguridad de datos, ID de publicidad «No», clasificación IARC y público objetivo quedaron guardados. Play muestra «Cambios en la etapa de revisión»; todavía no hay confirmación de aprobación ni disponibilidad de la prueba cerrada. [Política Android](https://certiva-landing.vercel.app/privacidad-app) · [Materiales y pendientes](landing/play-store-assets/README.md).

Acceso interno: tras confirmar el registro, `/probar` muestra un enlace a la prueba interna 0.5. La sincronización de verificadores está programada cada 5 minutos y depende del equipo conectado, de su ejecución correcta y del cupo de 100; el registro no confirma por sí solo acceso a Play. [Estado y comprobaciones](landing/INTERNAL-ACCESS-REVIEW.md).

Actualización posterior de Play: la tarea propietaria confirmó que la **prueba abierta 0.5** también fue enviada a revisión para **Colombia y Panamá**. No hay aprobación ni disponibilidad de descarga confirmadas para ese canal.
