...
(async () => {
  ...
  for (const e of estado) console.log(`  ${e.disponible ? "✓" : "✗"} ${e.label.padEnd(28)} ${e.constName.padEnd(46)} ${e.disponible ? gb(e.bytes) : "NO EXISTE"} ${e.enCache ? "(en caché)" : e.parcial ? "(PARCIAL)" : ""}`);
  let total = 0;
  for (const e of quiero) {
    ...
    if (s.parcial) {
      ...
      for (const id of s.archivos) for (const f of require("node:fs").readdirSync(modelos.CACHE_DIR)) if (f.endsWith(id)) { require("node:fs").unlinkSync(require("node:path").join(modelos.CACHE_DIR, f)); console.log(`  parcial borrado: ${f}`); }
    }
