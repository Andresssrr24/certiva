...
(async () => {
  ...
  const muestra = []; let i = 1;
  while (muestra.length < n) { for (const t of tipos) { const f = `${t}-${String(i).padStart(2, "0")}.png`; if (todos.includes(f) && muestra.length < n) muestra.push(f); } i++; if (i > 8) break; }
  const motor = new Motor();
  let ok = 0, enlacesOk = 0, conEnlaces = 0;
  for (const f of muestra) {
    const id = f.replace(/\.png$/, ""); const v = verdad[id];
    const r = await motor.extraerCaptura(path.join(dir, f));
    const c = r.captura;
    const s = reglas.evaluar(c, motor.banco); const ver = reglas.veredictoPorReglas(s);
    const dv = (v.enlaces || []).map(reglas.dominioDe).sort().join(","), dc = (c.enlaces || []).map(reglas.dominioDe).sort().join(",");
    if (v.enlaces.length) { conEnlaces++; if (dv === dc) enlacesOk++; }
    const bien = ver === v.esperado.veredicto; if (bien) ok++;
    console.log(`\n=== ${id} · OCR ${r.ttft} ms · ${bien ? "✓" : "✗"} reglas ${ver} (esperado ${v.esperado.veredicto}) [${s.map((x) => x.tipo).join(", ")}]`);
    console.log(`  canal ${c.canal}${c.canal === v.canal ? "" : " ✗ " + v.canal} · remitente «${c.remitente}» (${v.remitente}) · enlaces ${dv === dc ? "✓" : "✗"} ${JSON.stringify(c.enlaces)} · tel ${JSON.stringify(c.telefonos)} · montos ${JSON.stringify(c.montos)}`);
