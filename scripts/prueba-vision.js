...
(async () => {
  ...
  const fraude = todos.filter((f) => f.startsWith("fraude")),
    legit = todos.filter((f) => f.startsWith("legitimo"));
  const muestra = [];
  for (let i = 0; muestra.length < n && (fraude[i] || legit[i]); i++) {
    if (fraude[i]) muestra.push(fraude[i]);
    if (legit[i] && muestra.length < n) muestra.push(legit[i]);
  }
  ...
  for (const f of muestra) {
    ...
    if (!r.captura) {
      console.log("  ✖ JSON inválido:", r.crudo.slice(0, 200));
      continue;
    }
    const c = r.captura;
    const enlacesOk =
      JSON.stringify((c.enlaces || []).map((x) => reglas.dominioDe(x)).sort()) ===
      JSON.stringify((v.enlaces || []).map((x) => reglas.dominioDe(x)).sort());
    const s = reglas.evaluar(c, motor.banco);
    const ver = reglas.veredictoPorReglas(s);
    const ok = ver === v.esperado.veredicto;
    if (ok) aciertos++;
    console.log(
      `  canal ${c.canal} (${v.canal}) · remitente «${c.remitente}» (${v.remitente}) · enlaces ${enlacesOk ? "✓" : "✗ " + JSON.stringify(c.enlaces)} · pide ${c.pide_datos_sensibles} (${v.pide_datos_sensibles}) · urgencia ${c.urgencia} (${v.urgencia})`,
    );
    console.log(`  texto: ${c.texto.slice(0, 140)}${c.texto.length > 140 ? "…" : ""}`);
    console.log(
      `  reglas sobre la extracción: ${ver} ${ok ? "✓" : "✗ esperado " + v.esperado.veredicto} [${s.map((x) => x.tipo).join(", ")}]`,
    );
  }
