...
const IDS = ["fraude-bloqueo_enlace-01", "legitimo-alerta_transaccion-01", "fraude-ejecutivo_whatsapp-01", "legitimo-correo_estado_cuenta-01", "fraude-pide_codigo-01", "legitimo-otp_legitimo-01"];
const PREGUNTAS = [
  ...
  ["pide", "Does the message ask the reader to send, reply with or confirm a password, PIN or verification code? Answer YES or NO."],
...
async function corre(S, modelId, ruta, prompt) {
  const t0 = Date.now();
  const run = S.completion({ modelId, stream: true, history: [{ role: "user", content: prompt, attachments: [{ path: ruta }] }] });
  for await (const _ of run.events) { /* */ }
  const f = await run.final; return { texto: String(f.contentText || "").trim().replace(/\s+/g, " "), ms: Date.now() - t0 };
}
(async () => {
  const e = modelos.entrada("vision", process.argv[2] || "visionpsy-flash"); const S = await modelos.sdk();
  const modelId = await S.loadModel(await modelos.argsCarga(e, { ctxSize: 4096 }));
  const verdad = require("../data/verdad.json");
  console.log(`########## ${e.label}`);
  for (const id of IDS) {
    const v = verdad[id]; const ruta = `${__dirname}/../data/capturas/${id}.png`;
    ...
    let ms = 0;
    for (const [k, p] of PREGUNTAS) { const r = await corre(S, modelId, ruta, p); ms += r.ms; console.log(`  ${k.padEnd(9)} ${r.texto.slice(0, 90)}`); }
